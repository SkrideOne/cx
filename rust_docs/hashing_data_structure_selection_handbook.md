# Best‑Practice Methodology for Hashing and Data‑Structure Selection in Rust nightly 1.90 (2025)

## 1 Background

The Rust nightly 1.90 release (branched from master on **1 Aug 2025**) uses the same core data structures as the stable standard library. However, the nightly release includes additional experimental APIs (e.g. `Vec::pop_if`, `try_with_capacity`, `VecDeque::pop_front_if`) and exposes the underlying **hashbrown‑based SwissTable** implementation for `HashMap` and `HashSet`. The guidance below targets **Best Practice 2025**: *avoid premature optimisation but know when to choose a faster hasher or alternative collection to meet performance, quality and security requirements*.
...
Because `HashMap` uses random seeds, it cannot be constructed at compile‑time. In nightly 1.90 you must either:
...
| **Dynamic array** (random access, contiguous)                     | `Vec<T>`                                                                  | Provides fast indexing and good cache locality (nnethercote.github.io). Use `reserve`/`with_capacity` to allocate upfront. To remove items efficiently without preserving order, use `swap_remove` (*O(1)*) instead of `remove` (*O(n)*); use `retain` when removing many items. Nightly 1.90 adds `pop_if` and `try_with_capacity`. |
| Queue or double‑ended queue                                       | `VecDeque<T>`                                                             | Ring buffer; supports amortised *O(1)* pushes/pops at both ends and *O(1)* indexing (doc.rust-lang.org). Use `make_contiguous` before sorting or slicing. Nightly adds `pop_front_if`/`pop_back_if` and `try_with_capacity`.                                                                                                         |
